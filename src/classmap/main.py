...
def main():
    ...
    read_from_file_or_manual = input(
        "Load pupils from file? (y/n): ").strip().lower()

    # Create classroom based on user choice
    if read_from_file_or_manual == 'y':
        classroom = create_classroom_from_file()
    else:
        classroom = create_classroom_from_console()
