...
class Classroom:
    ...
    def display_map(self, filepath: str = "classroom_map.txt", printout: bool = False) -> None:
        # Create a 2D list representing the classroom layout
        # Pretty print the classroom map with pupils' positions
                
        
        classroom_array: list[list[str | None]] = [[None for _ in range(self._num_columns)] for _ in range(self._num_rows)]
        
        for pupil in self._pupils:
            # Skip pupils without a set position
            try: row, col = pupil.get_position()
            except ValueError: continue
                
            # Check for position conflicts
            if (occupant := classroom_array[row][col]) is not None:
                raise ValueError(f"Position ({row}, {col}) is already occupied by {occupant} trying to add pupil {pupil.name}.")
        ...
        width: int = 1 + max((len(p.name) for p in self._pupils), default=5) + 1
    ...
    def arrange_seating(self) -> None:
        ...
        for row in range(self._num_rows):
            for col in range(self._num_columns):
                ...
                if not pupils_not_seated: return 
